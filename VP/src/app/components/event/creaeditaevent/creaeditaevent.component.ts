...
export class CreaeditaeventComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.uS.listarPorRol(this.rolPsicologo).subscribe((data: User[]) => {
      this.listaPsicologos = data;
    });

    this.uS.listarPorRol(this.rolVeterano).subscribe((data: User[]) => {
  ...
  aceptar(): void{
    if(this.form.valid){
      ...
      this.event.verterano = this.form.value.hveterano;
      this.event.psicologo = this.form.value.hpsicologo
      this.event.eventType = this.form.value.heventType
